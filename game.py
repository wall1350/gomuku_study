...
class Board(object):
    ...
    def has_a_winner(self):
        ...
        if (w in range(width - n + 1) and
            len(set(states.get(i, -1) for i in range(self.last_move, self.last_move + n))) == 1):
            return True, player
        #left
        if (w in range(n - 1, width) and
            len(set(states.get(i, -1) for i in range(self.last_move, self.last_move - n, -1))) == 1):
            return True, player
        #down
        if (h in range(height - n + 1) and
            len(set(states.get(i, -1) for i in range(self.last_move, self.last_move + n * width, width))) == 1):
            return True, player
        #up
        if (h in range(n - 1, height) and
            len(set(states.get(i, -1) for i in range(self.last_move, self.last_move - n * width, -width))) == 1):
            return True, player
        #left_top to right_bottom
        if (w in range(width - n + 1) and h in range(height - n + 1) and
            len(set(states.get(i, -1) for i in range(self.last_move, self.last_move + n * (width + 1), width + 1))) == 1):
            return True, player
        #right_bottom to left_top
        if (w in range(n - 1, width) and h in range(n - 1, height) and
            len(set(states.get(i, -1) for i in range(self.last_move, self.last_move + n * (-width - 1), -width - 1))) == 1):
            return True, player
