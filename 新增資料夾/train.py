...
class TrainPipeline():
    def __init__(self):
        ...
        self.kl_targ = 0.02
        self.check_freq = 250 #1000
        self.game_batch_num = 10000
        self.best_win_ratio = 0.0
        # num of simulations used for the pure mcts, which is used as the opponent to evaluate the trained policy
        self.pure_mcts_playout_num = 3000 #3000

        # start training from a new policy-value net
        self.policy_value_net = PolicyValueNet("0711")
    ...
    def policy_evaluate(self, n_games=10):
        ...
        for i in range(n_games):
            if i%2==0:
                winner = self.game.start_play(current_mcts_player, pure_mcts_player, start_player=0, is_shown=0)
            else:
                winner = self.game.start_play(pure_mcts_player, current_mcts_player, start_player=0, is_shown=0)
                winner = 3 - winner
        ...
        try:
            for i in range(self.game_batch_num):
                ...
                if (i+1) % self.check_freq == 0:
                    ...
                    self.policy_value_net.saver.save(self.policy_value_net.sess, './model/tf_policy_15_15_5_model')
                    ...
                    if win_ratio > self.best_win_ratio:
                        ...
                        self.policy_value_net.saver.save(self.policy_value_net.sess, './model/tf_policy_15_15_5_model') # update the best_policy
                        if self.best_win_ratio == 1.0 and self.pure_mcts_playout_num < 10000:
                            self.pure_mcts_playout_num += 1000
                            print("self.pure_mcts_playout_num = {}".format(self.pure_mcts_playout_num))
                            self.best_win_ratio = 0.0
